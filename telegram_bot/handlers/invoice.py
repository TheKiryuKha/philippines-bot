...
async def create_data(update: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    message = (
        ...
        f"<b>"
        f"ФИО (как к тебе обращаться)\n"
        f"Номер телефона (только цифры),\n"
        f"Адрес доставки\n\n"
        f"</b>"
...
async def pay(update: Message, state: FSMContext, bot: Bot):
    ...
    await bot.send_message(
        chat_id=update.from_user.id,
        text=f"Отлично! Благодарим за оплату. Скоро бот отправит вам данные для получения. По любым вопросам можете обращаться в поддержку"
    )
